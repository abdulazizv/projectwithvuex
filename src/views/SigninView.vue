    ...
    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
      ...
      <div
        ...
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <p class="text-center text-red-400" v-if="authStatus">
            {{ authStatus }}
          </p>
          <form class="space-y-4 md:space-y-6" action="/" @submit="useAuth">
            <div>
              <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your
                username</label>
              <input type="text" name="username" id="username"
                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="name@gmail.com" required="" v-model="username" />
            </div>
            <div>
  ...
  methods: {
    useAuth(e) {
      ...
      if (params.username.length == 0 || params.password.length == 0) {
        ...
        this.$store.dispatch("LOGIN_ADMIN", params);
      }
    },
  },
  computed: {
    authStatus() {
      return this.$store.state.login.logMessage;
    },
  },
};
</script>
